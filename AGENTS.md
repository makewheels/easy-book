...
2. **预约系统**: 预约时立即扣减课程次数，取消时恢复次数
3. **预约状态**: 支持scheduled（待上课）、cancel（已取消）状态
4. **冲突解决**: 1v1课程在同一时间段不能重叠

## API端点结构

- **学员管理**: `/api/students/` - 完整的CRUD操作
- **预约管理**: `/api/appointments/` - 创建、查询、取消预约，预约时扣减课程
...
### 文件管理和清理规范
- **截屏文件管理**:
  - AI助手应优先使用内存中的截图数据，仅在必要时保存到文件
  - 如需保存截图，应在完成任务后立即清理临时文件
  - 避免在项目根目录创建无用的txt、png等临时文件
- **定期清理**: 开发过程中应及时清理不必要的临时文件
- **文件组织**: 所有项目文件应放在合适的目录中，避免散落在根目录
